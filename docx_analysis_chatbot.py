...
# Database setup
conn = sqlite3.connect("docx_analysis_chat_history.db", check_same_thread=False)
cursor = conn.cursor()
cursor.execute("""
...
### 1️⃣ Extract Text and Images from DOCX ###
def get_docx_content(docx_files):
    text_content = ""
    image_list = []
...
def generate_response(context, user_question):
    ...
    if not context:
        return "I need the document content to answer your question. Please upload a DOCX file first."
...
def main():
    st.set_page_config("Document Analysis", layout="wide")
    st.title("📄 Document Analysis Chatbot")
    st.subheader("Upload DOCX files with text and images")
            ...
            if uploaded_files:
                with st.spinner("Processing documents..."):
                    try:
                        ...
                        text_content, image_list = get_docx_content(uploaded_files)
                ...
                st.warning("Please upload DOCX files first.")
